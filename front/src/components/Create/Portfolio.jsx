...
const Edit = () => {
    ...
    const onSubmit = data => {
        ...
        axios.post('http://localhost:4000/api/portfolio', formData)
            .then(res => {
                console.log(res.data)
                navigate('/')
            }).catch(err => {
                alert(err.message + ' - Erreur lors de la création du portfolio')
            });
    }

    return (
        <>
            <MainTitle>Créer une partie de la section Portfolio</MainTitle>
